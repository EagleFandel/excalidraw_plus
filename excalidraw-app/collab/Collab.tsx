...
import {
  isSavedToCollabStorage,
  loadFilesFromCollabStorage,
  loadSceneFromCollabStorage,
  saveFilesToCollabStorage,
  saveSceneToCollabStorage,
} from "./collab-storage-api";
...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  constructor(props: CollabProps) {
    ...
    this.fileManager = new FileManager({
      getFiles: async (fileIds) => {
        ...
        return loadFilesFromCollabStorage(roomId, roomKey, fileIds);
      },
      saveFiles: async ({ addedFiles }) => {
        ...
        const { savedFiles, erroredFiles } = await saveFilesToCollabStorage({
          roomId,
  ...
  private beforeUnload = withBatchedUpdates((event: BeforeUnloadEvent) => {
    ...
    if (
      ...
      (this.fileManager.shouldPreventUnload(syncableElements) ||
        !isSavedToCollabStorage(this.portal, syncableElements))
    ) {
  ...
  saveCollabRoomToStorage = async (
    ...
    try {
      const storedElements = await saveSceneToCollabStorage(
        this.portal,
        syncableElements,
  ...
  private initializeRoom = async ({
    ...
    if (fetchScene && roomLinkData && this.portal.socket) {
      ...
      try {
        const elements = await loadSceneFromCollabStorage(
          roomLinkData.roomId,
